...
colnames(sim_power) <- c("Number_Obs","quantile","st_error")
#Reset sample_means
sample_means <- NULL
sample_means = rep(0,total_samps)
quartileVector = rep(0, 100)


# for each sample size ake sample and find mean 100 times for each one
for(r in 1:100){
for (j in 1:obs_count) {
  for (i in 1:total_samps) {
    # do qantile on means here
  subsample <- c(sample(X[,1], sim_power[j,1])) # sim poiwer is size od sample to choose
  sample_means[i] = mean(subsample) 
  }
  sim_power[j,2] <- quantile(sample_means, .05) #mean(sample_means)
  sim_power[j,3] <- st_dev/(sim_power[j,1]^.5)
}
  x <- filter(sim_power, quantile > 9)[1, 1]
  quartileVector[r] = x[1]

}
quartileVector <- as.data.frame(quartileVector)
v <- summary(quartileVector)

filter(sim_power, quantile > 9)
...
#Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated t#he plot.
